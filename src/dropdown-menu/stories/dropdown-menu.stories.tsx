...
export const nested = () => {
  ...
  const NestedMenuItem = (props: NestedMenuProps) => {
    const anchor: DropdownMenuProps['anchor'] = ({active}, ariaProps = {}) => (
      <Group
        ...
        {...ariaProps}
      >
        <span className={styles.nestedMenuTitle}>{props.title}</span>

        <Icon glyph={chevronRight} className={styles.chevronIcon} />
      </Group>
    );
  ...
  const data: ListDataItem[] = menu.reduce((acc, menuItem) => {
    ...
    const secondLevelRoot: ListDataItem = {
      ...
      key: menuItem.key,
      template: (
        <button type='button' className={styles.nestedMenuButton} onClick={e => e.stopPropagation()}>
          <NestedMenuItem
