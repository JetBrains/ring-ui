...
export const inputWithoutControls: StoryFn<SingleSelectAttrs> = args => <Select {...args} className={styles.wrapper} />;
...
withCustomItemsAndAnAddItemButton.args = {
  data: [...Array(100)].map((elem, idx) => {
    ...
    return {
      label,
      key: label,
      template: <span className={styles.label}>{label}</span>,
...
export const selectInPopup: StoryFn<SingleSelectAttrs> = args => (
  <Dropdown anchor='Open dropdown'>
    <Popup className={styles.popupTestClass} maxHeight={100}>
      <Select {...args} />
    </Popup>
  </Dropdown>
);
