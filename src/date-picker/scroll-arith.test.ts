...
describe('ScrollArith with months', () => {
  ...
  describe('getScrollTop', () => {
    it('should calculate scrollTop for a mid date', () => {
      ...
      expect(arith.getScrollTop(items, midDate)).to.be.closeTo(expectedScrollTop, 1e-7);
    });
    ...
    it('should calculate scrollTop when scrollDate is before the first item', () => {
      ...
      expect(arith.getScrollTop(items, beforeDate)).to.be.closeTo(expectedScrollTop, 1e-7);
    });
  ...
  describe('getScrollDate', () => {
    it('should return scrollDate between items 3 and 4', () => {
      ...
      expect(Number(arith.getScrollDate(items, scrollTop))).to.be.closeTo(Number(expectedScrollDate), 3600_000);
    });

    it('should return last item for very high scrollTop', () => {
      expect(Number(arith.getScrollDate(items, ITEM_HEIGHT * 100))).to.equal(Number(items[items.length - 1]));
    });
  });
